        ...
        public async Task<List<RequestResDto>> GetAllRequestsByAgentId(string userId)
        {
            ...
            if (exReq.Count > 0)
            {
                foreach (var exr in exReq)
                {
                    ...
                    var createdBy1 = await _userRepository.GetByIdAsync(createdByObjectId1) ?? throw new Exception("Invalid User ID");
                    UserDto createdByDto1 = new(createdBy1.Id.ToString(), createdBy1.Name, createdBy1.Role, createdBy1.NIC, createdBy1.ImagePath, createdBy1.ContactNo, createdBy1.IsActive, createdBy1.IsSendActiveStatus, createdBy1.Email);
                    ...
                    var agent = await _userRepository.GetByIdAsync(agentObjectId) ?? throw new Exception("Invalid User ID");
                    UserDto agentDto = new(agent.Id.ToString(), agent.Name, agent.Role, agent.NIC, agent.ImagePath, agent.ContactNo, agent.IsActive, agent.IsSendActiveStatus, agent.Email);
